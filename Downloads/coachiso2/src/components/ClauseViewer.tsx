...
import ManualSIG from './ManualSIG';
    ...
    <div className="min-h-screen bg-[#050505] text-gray-200 font-sans py-20 px-4 selection:bg-red-500/30">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-12">
          ...
          <p className="text-red-400 font-medium text-lg">Visor Dinámico de Normativas & Consola de Control de Sistemas de Gestión (SIG)</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="p-4 border border-white/5 bg-white/[0.02] rounded-2xl mb-6">
              ...
              <div className="space-y-1">
                <button 
                  ...
                  className={`w-full text-left px-4 py-3 rounded-xl transition text-sm font-medium ${activeClause === '4.4' && activeNorm === 'ISO 9001:2015' ? 'bg-red-600 text-white shadow-lg' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}
                >
                  Cláusula 4.4: Sistema de Gestión
                </button>
                <button 
                  onClick={() => { setActiveClause('5.3'); setActiveNorm('ISO 9001:2015'); }}
                  className={`w-full text-left px-4 py-3 rounded-xl transition text-sm font-medium ${activeClause === '5.3' && activeNorm === 'ISO 9001:2015' ? 'bg-red-600 text-white shadow-lg' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}
                >
                  Cláusula 5.3: Roles y Autoridades
                </button>
                <button 
                  onClick={() => { setActiveClause('7.5'); setActiveNorm('ISO 9001:2015'); }}
                  className={`w-full text-left px-4 py-3 rounded-xl transition text-sm font-medium ${activeClause === '7.5' && activeNorm === 'ISO 9001:2015' ? 'bg-red-600 text-white shadow-lg' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}
                >
                  Cláusula 7.5: Información Documentada
                </button>
                <button 
                  onClick={() => { setActiveClause('8.1'); setActiveNorm('ISO 9001:2015'); }}
                  className={`w-full text-left px-4 py-3 rounded-xl transition text-sm font-medium ${activeClause === '8.1' && activeNorm === 'ISO 9001:2015' ? 'bg-red-600 text-white shadow-lg' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}
                >
          ...
          {/* Área de Visualización (ISOImage Render Engine o ManualSIG Dinámico) */}
          <div className="lg:col-span-9 space-y-8">
            
            {activeClause === '4.4' && activeNorm === 'ISO 9001:2015' ? (
              <div className="space-y-8 animate-fadeIn">
                {/* 1. SECCIÓN DEDICADA AL MATERIAL DE APOYO REGISTRADO */}
                <div className="glass p-8 rounded-3xl border border-white/5">
                  <h2 className="text-2xl font-bold text-white mb-4">Material de Apoyo Técnico Verificado</h2>
                  <p className="text-gray-400 mb-6 leading-relaxed text-sm">
                    A continuación se presenta el activo verificado para la validación de la cláusula seleccionada. Este recurso es utilizado oficialmente durante las sesiones de auditoría de Coach-ISO en combinación con la consola interactiva integrada adjunta a continuación.
                  </p>
                  {mappedImage && <ISOImage id={mappedImage.id} />}
                </div>

                {/* 2. EL SISTEMA INTEGRADO TOTAL DEMANDADO POR EL CLIENTE */}
                <ManualSIG />
              </div>
            ) : (
              <div className="glass p-8 rounded-3xl border border-white/5">
