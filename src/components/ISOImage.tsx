...
import SGCProcessMap from './SGCProcessMap';
...
function ISOImage({ 
  ...
  return (
    <>
      ...
      >
        {/* Contenedor de la Imagen o del Mapa Técnico */}
        <div 
          className={`relative w-full overflow-hidden bg-black/60 ${id !== 'mapa-procesos-9001' ? 'cursor-pointer' : ''}`}
          onClick={id !== 'mapa-procesos-9001' ? () => setIsLightboxOpen(true) : undefined}
        >
          {id === 'mapa-procesos-9001' ? (
            <div className="w-full h-auto p-2 sm:p-4 bg-[#060606] border-b border-white/5">
              <SGCProcessMap />
            </div>
          ) : (
            <img 
          ...
          {(normaRelacionada || clausula) && id !== 'mapa-procesos-9001' && (
